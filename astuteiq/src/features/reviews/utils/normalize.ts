...
const CHECKS: { title: string; category: string; messages: Partial<Record<ReviewStatus, string>> }[] = [
  {
...
function weightedStatus(): ReviewStatus {
  const r = Math.random()
...
export function generateMockReview(mode: ReviewMode, id?: string): ReviewRecord {
  const reviewId = id ?? `rev_${Date.now()}`
  const findings: ReviewFinding[] = CHECKS.map((c, i) => {
...
export function generateMockHistory(page: number, limit: number): ReviewHistoryResponse {
  const fileNames = [
  ...
  const reviews: ReviewRecord[] = fileNames.slice(0, limit).map((name, i) => {
    const score = 60 + Math.floor(Math.random() * 40)
    return {
      id:          `rev_hist_${page}_${i}`,
      userId:      'usr_002',
      fileName:    name,
      fileSize:    400_000 + Math.floor(Math.random() * 600_000),
      ...
      completedAt: new Date(Date.now() - i * 86_400_000 * 2 + 60_000).toISOString(),
    }
  })
