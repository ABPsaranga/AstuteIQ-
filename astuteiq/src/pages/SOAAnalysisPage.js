...
/* ============================================================================
   TOKEN BUDGET
============================================================================ */
function estimateTokens(content, type) {
    if (type === 'pdf_b64')
        return Math.round((content.length * 0.75) / 3.5);
    return Math.round(content.length / 4);
}
function getStage1SuppCount(soaDoc, refDoc, suppDocs) {
    const BUDGET = 170000;
    const sysCost = 4000;
    const soaCost = estimateTokens(soaDoc.content, soaDoc.docType);
    const refCost = refDoc ? Math.min(estimateTokens(refDoc.content, refDoc.docType), 10000) : 0;
    let total = sysCost + soaCost + refCost;
    let count = 0;
    for (const doc of suppDocs) {
        const t = Math.min(estimateTokens(doc.content, doc.docType), 30000);
        if (total + t > BUDGET)
            break;
        total += t;
        count++;
...
function truncateRefSoa(text, maxChars) {
    if (text.length <= maxChars)
        return text;
    ...
    for (const line of lines) {
        ...
        else if (sectionLines < 3 && trimmed.length > 0) {
            result.push(line);
            sectionLines++;
            charCount += line.length;
        }
...
`;
function buildSystemPrompt(mode, hasRef) {
    if (mode === 'quick') {
        return ('You are a senior Australian financial planning compliance expert at AstuteIQ. ' +
...
function buildStage2SystemPrompt() {
    return ('You are a financial planning compliance reviewer at AstuteIQ. Stage 2 cross-check only. ' +
...
/* ============================================================================
   CONTENT BUILDERS
============================================================================ */
function buildContentParts(soaDoc, refDoc, suppDocs, suppCount) {
    const parts = [{ type: 'text', text: 'Please conduct Stage 1 of a comprehensive compliance review:' }];
    if (soaDoc.docType === 'pdf_b64') {
        ...
        parts.push({ type: 'text', text: `--- NEW SOA BEING REVIEWED ---\n${truncateText(soaDoc.content, 120000)}` });
    }
    if (refDoc) {
        if (refDoc.docType === 'pdf_b64') {
            ...
            parts.push({ type: 'text', text: `--- REFERENCE SOA ---\n${truncateRefSoa(refDoc.content, 40000)}` });
        }
    }
    suppDocs.slice(0, suppCount).forEach((doc, i) => {
        ...
        if (doc.docType === 'pdf_b64') {
            ...
            parts.push({ type: 'text', text: `${label}\n${truncateText(doc.content, 30000)}` });
        }
    });
    return parts;
}
function buildStage2Parts(soaDoc, remainingSupp) {
    const parts = [{ type: 'text', text: 'Stage 2: Cross-check the SOA against these additional supporting documents:' }];
    if (soaDoc.docType === 'pdf_b64') {
...
function repairAndParseJSON(text) {
    ...
    try {
        ...
        if (sm) {
            ...
            if (lb > 0)
                arrStr = arrStr.substring(0, lb + 1) + ']';
            ...
            if (dm)
                try {
                    r.docs_reviewed = JSON.parse(dm[1]);
                }
                catch { /* ignore */ }
            return r;
        }
...
function mergeResults(r1, r2) {
    ...
    for (const c of allChecks) {
        ...
        if (!key)
            continue;
        if (!byId[key] || (c.note || '').length > (byId[key].note || '').length)
            byId[key] = c;
    }
...
function loadFeedbackStore() {
    try {
        const s = localStorage.getItem(FEEDBACK_LS_KEY);
...
function saveFeedbackStore(store) {
    try {
        localStorage.setItem(FEEDBACK_LS_KEY, JSON.stringify(store));
    }
    catch { /* ignore */ }
}
/* ============================================================================
...
/* ============================================================================
   WORD EXPORT
============================================================================ */
async function exportToWord(result, overrides) {
    ...
    try {
        ...
        function makeSummaryCell(label, value, fill) {
        ...
        function makeFindingRow(check) {
            ...
            if (ov) {
                ...
                if (ov.newStatus !== ov.originalStatus) {
                    noteLines.push(new TextRun({
                        text: `\nOverridden: ${statusLabel(ov.originalStatus)} → ${statusLabel(ov.newStatus)}`,
                        bold: true,
                        color: 'CC8800',
                    }));
                }
                            ...
                            new Paragraph({
                                ...
                                children: [
                                    new TextRun({
                                        ...
                                        bold: true,
                                        color: STATUS_COLOURS[effectiveStatus] ||
        ...
        const findingsSections = Object.entries(groupedChecks).flatMap(([area, checks]) => [
            new Paragraph({
                text: area,
        ...
        const overriddenChecks = effectiveChecks.filter((c) => overrides[c.id]);
            ...
            sections: [
                {
                    children: [
                        ...
                        new Paragraph({
                            children: [
                                new TextRun({
                                    text: 'This report was generated by AstuteIQ AI. It is a compliance assistance tool only and does not constitute legal or financial advice. All findings must be reviewed by a qualified compliance professional before the SOA is submitted.',
                                    italics: true,
                                }),
                            ],
                        }),
        ...
        anchor.href = url;
        anchor.download = `AstuteIQ_${safeClient}_${result.date || 'report'}.docx`;
...
function UploadZone({ label, num, desc, formats, file, multiple, files, onDrop, onRemove }) {
    ...
    return (_jsxs("div", { className: `relative flex flex-col gap-3 border-2 border-dashed rounded-2xl p-4 min-h-[220px] transition-all duration-300 ${hasFile ? 'border-[#6B2FD9] bg-[#6B2FD9]/10 shadow-lg shadow-[#6B2FD9]/10'
            : isDragActive ? 'border-[#A78BFA] bg-[#6B2FD9]/10'
                : 'border-slate-800 bg-[#0f0f1a] hover:border-[#6B2FD9]/40'}`, children: [_jsxs("div", { className: "flex items-center gap-2", children: [_jsx("span", { className: "w-5 h-5 rounded-full bg-[#6B2FD9] flex items-center justify-center text-white text-xs font-bold shrink-0", children: num }), _jsx("span", { className: `text-sm font-semibold ${hasFile ? 'text-[#A78BFA]' : 'text-slate-200'}`, children: label })] }), _jsx("p", { className: "text-xs text-slate-500", children: desc }), _jsx("p", { className: "text-xs text-slate-600 italic", children: formats }), !multiple && file && (_jsxs("div", { className: "flex items-center gap-2 bg-[#141421] border border-slate-800 rounded-lg px-3 py-2 mt-auto", children: [_jsx(FileText, { size: 13, className: "text-[#A78BFA]" }), _jsx("span", { className: "text-xs text-slate-200 flex-1 truncate", children: file.name }), _jsx("span", { className: "text-xs text-slate-500", children: formatBytes(file.size) }), _jsx("button", { onClick: () => onRemove(), className: "text-slate-600 hover:text-[#FF6B6B]", children: _jsx(X, { size: 13 }) })] })), multiple && files && files.length > 0 && (_jsx("div", { className: "space-y-1", children: files.map((f) => (_jsxs("div", { className: "flex items-center gap-2 bg-[#141421] border border-slate-800 rounded-lg px-3 py-1.5", children: [_jsx("span", { className: "text-xs text-slate-200 flex-1 truncate", children: f.name }), _jsx("button", { onClick: () => onRemove(f.id), className: "text-slate-600 hover:text-[#FF6B6B]", children: _jsx(X, { size: 12 }) })] }, f.id))) })), _jsxs("div", { ...getRootProps(), className: "flex-1 flex flex-col items-center justify-center gap-2 cursor-pointer rounded-lg p-3 hover:bg-[#6B2FD9]/5 transition-colors", children: [_jsx("input", { ...getInputProps() }), _jsx(Upload, { size: 20, className: isDragActive ? 'text-[#A78BFA]' : 'text-slate-600' }), _jsx("p", { className: "text-xs text-slate-500 text-center", children: isDragActive ? 'Drop files here' : 'Click or drag to upload' })] })] }));
}
/* ============================================================================
   OVERRIDE PANEL
============================================================================ */
function OverridePanel({ checkId, currentOverride, onSave, onClear, onClose, originalStatus, }) {
    const [newStatus, setNewStatus] = useState(currentOverride?.newStatus ?? originalStatus);
    const [comment, setComment] = useState(currentOverride?.comment ?? '');
    const statuses = [
        { value: 'pass', label: 'PASS', color: '#2DD4A0' },
        { value: 'warning', label: 'WARNING', color: '#FFB347' },
        { value: 'fail', label: 'FAIL', color: '#FF6B6B' },
        { value: 'na', label: 'N/A', color: '#6b7280' },
    ];
    return (_jsxs("div", { className: "mt-3 p-4 rounded-xl border border-[#FFB347]/30 bg-[#FFB347]/5 space-y-3", children: [_jsxs("p", { className: "text-xs font-semibold text-[#FFB347] uppercase tracking-wide flex items-center gap-1.5", children: [_jsx(Flag, { size: 11 }), " Override finding"] }), _jsx("div", { className: "flex gap-2 flex-wrap", children: statuses.map(({ value, label, color }) => (_jsx("button", { onClick: () => setNewStatus(value), className: "px-3 py-1 rounded-lg text-xs font-semibold border transition-all", style: { borderColor: newStatus === value ? color : `${color}30`, background: newStatus === value ? `${color}20` : 'transparent', color: newStatus === value ? color : '#6b7280' }, children: label }, value))) }), _jsx("textarea", { value: comment, onChange: (e) => setComment(e.target.value), placeholder: "Add a comment (optional)...", rows: 2, className: "w-full px-3 py-2 rounded-lg bg-[#0B0B14] border border-slate-700 text-xs text-slate-200 placeholder-slate-600 focus:outline-none focus:border-[#FFB347]/50 resize-none" }), _jsxs("div", { className: "flex gap-2", children: [_jsx("button", { onClick: () => { onSave(checkId, { originalStatus, newStatus, comment }); onClose(); }, className: "px-3 py-1.5 rounded-lg text-xs font-semibold text-white", style: { background: '#FFB347' }, children: "Save override" }), currentOverride && (_jsx("button", { onClick: () => { onClear(checkId); onClose(); }, className: "px-3 py-1.5 rounded-lg text-xs font-medium border border-slate-700 text-slate-400 hover:text-white transition-colors", children: "Clear" })), _jsx("button", { onClick: onClose, className: "px-3 py-1.5 rounded-lg text-xs font-medium text-slate-500 hover:text-slate-300 transition-colors", children: "Cancel" })] })] }));
}
/* ============================================================================
   FINDING ROW
============================================================================ */
function FindingRow({ check, override, onSaveOverride, onClearOverride, }) {
    const [showOverride, setShowOverride] = useState(false);
    const effectiveStatus = override?.newStatus ?? check.status;
    const isFlagged = !!override;
    return (_jsxs("div", { className: `py-3 border-b border-slate-800/60 last:border-0 ${isFlagged ? 'bg-[#FFB347]/5 rounded-xl px-3 -mx-3' : ''}`, children: [_jsxs("div", { className: "flex items-start gap-3", children: [_jsxs("div", { className: "mt-0.5 shrink-0", children: [effectiveStatus === 'pass' && _jsx(CheckCircle, { size: 15, className: "text-[#2DD4A0]" }), effectiveStatus === 'fail' && _jsx(XCircle, { size: 15, className: "text-[#FF6B6B]" }), effectiveStatus === 'warning' && _jsx(AlertTriangle, { size: 14, className: "text-[#FFB347]" }), effectiveStatus === 'na' && _jsx(Minus, { size: 15, className: "text-slate-600" })] }), _jsxs("div", { className: "flex-1 min-w-0", children: [_jsxs("div", { className: "flex items-center gap-2 flex-wrap mb-1", children: [_jsx("span", { className: "text-xs font-mono text-slate-500", children: check.id }), _jsx(StatusBadge, { status: STATUS_MAP[effectiveStatus] ?? 'NA', size: "sm" }), isFlagged && override.newStatus !== override.originalStatus && (_jsx("span", { className: "text-xs text-slate-500 line-through font-mono", children: override.originalStatus.toUpperCase() })), _jsx("span", { className: "text-sm font-medium text-slate-200", children: check.label }), isFlagged && (_jsxs("span", { className: "inline-flex items-center gap-1 text-xs font-semibold text-[#FFB347] border border-[#FFB347]/30 bg-[#FFB347]/10 px-2 py-0.5 rounded-full", children: [_jsx(Flag, { size: 9 }), " Flagged"] }))] }), _jsx("p", { className: "text-xs text-slate-400 leading-relaxed", children: check.note }), isFlagged && override.comment && (_jsxs("p", { className: "text-xs text-[#FFB347]/80 italic mt-1", children: ["Comment: ", override.comment] })), _jsxs("button", { onClick: () => setShowOverride((v) => !v), className: `mt-2 inline-flex items-center gap-1.5 text-xs font-medium transition-colors ${isFlagged ? 'text-[#FFB347] hover:text-[#FFB347]/70' : 'text-slate-500 hover:text-[#FFB347]'}`, children: [_jsx(Flag, { size: 11 }), isFlagged ? 'Edit override' : 'Mark as incorrect', showOverride ? _jsx(ChevronUp, { size: 11 }) : _jsx(ChevronDown, { size: 11 })] })] })] }), showOverride && (_jsx(OverridePanel, { checkId: check.id, originalStatus: check.status, currentOverride: override, onSave: onSaveOverride, onClear: onClearOverride, onClose: () => setShowOverride(false) }))] }));
}
/* ============================================================================
   SCORE TILES
============================================================================ */
...
function ScoreTiles({ checks, overrides }) {
    const effective = checks.map((c) => overrides[c.id]?.newStatus ?? c.status);
    const count = (s) => effective.filter((x) => x === s).length;
    return (_jsx("div", { className: "grid grid-cols-4 gap-2", children: SCORE_TILES.map(([key, color, label]) => (_jsxs("div", { className: "rounded-xl p-3 text-center border", style: { background: `${color}10`, borderColor: `${color}25` }, children: [_jsx("p", { className: "text-xl font-bold font-mono", style: { color }, children: count(key) }), _jsx("p", { className: "text-xs font-semibold uppercase tracking-wide mt-0.5", style: { color }, children: label })] }, key))) }));
}
/* ============================================================================
   DIRECT API CALL (fallback when no backend session)
============================================================================ */
async function callAnthropicDirect(systemPrompt, contentParts, maxTokens, signal, onChunk) {
    const apiKey = import.meta.env.VITE_ANTHROPIC_API_KEY ?? '';
    if (!apiKey)
        throw new Error('No API key. Log in to use the backend, or set VITE_ANTHROPIC_API_KEY in .env for dev direct mode.');
    ...
    if (!parsed)
        throw new Error('Could not parse response. Try removing supporting documents or switching PDFs to Word format.');
    return parsed;
}
/* ============================================================================
   MAIN PAGE
...
export default function SOAAnalysisPage() {
    ...
    const [feedback, setFeedback] = useState(() => loadFeedbackStore());
    ...
    function saveOverride(checkId, override) {
    ...
    function clearOverride(checkId) {
        setOverrides((prev) => { const n = { ...prev }; delete n[checkId]; return n; });
        setFeedback((prev) => { const n = { ...prev }; delete n[checkId]; return n; });
    }
    const handleSoaDrop = useCallback(async (files) => {
        if (!files[0])
            return;
        setSoaDoc(await readFile(files[0], 'NEW SOA BEING REVIEWED'));
    }, []);
    const handleRefDrop = useCallback(async (files) => {
        if (!files[0])
            return;
        setRefDoc(await readFile(files[0], 'REFERENCE SOA'));
    }, []);
    const handleSuppDrop = useCallback(async (files) => {
        const existing = suppDocs.length;
        const docs = await Promise.all(files.slice(0, 10 - existing).map((file, i) => readFile(file, `SUPPORTING DOCUMENT ${existing + i + 1}`)));
        setSuppDocs((prev) => [...prev, ...docs]);
    }, [suppDocs]);
    async function runReview(reviewMode) {
        if (!soaDoc) {
        ...
        try {
            ...
            if (useBackend) {
                    ...
                    for (const event of events) {
                        ...
                        try {
                            ...
                            if (payload.step) {
                                ...
                                updateLiveReview(liveId, { progress: Math.min(payload.step * 18, 90) });
                            }
                            if (payload.error)
                                throw new Error(`Backend: ${payload.error}`);
                            if (payload.result) {
                            ...
                            if (pe?.message?.startsWith('Backend:'))
                                throw pe;
                ...
                result1 = await callAnthropicDirect(buildSystemPrompt(reviewMode, !!refDoc), parts, reviewMode === 'quick' ? 4000 : 10000, controller.signal, (chunk) => setStreamText((prev) => prev + chunk));
            }
            setStep(4);
            ...
            if (remainingSupp.length > 0) {
                ...
                if (useBackend) {
                    ...
                    try {
                        result2 = await callAnthropicDirect(buildStage2SystemPrompt(), buildStage2Parts(soaDoc, remainingSupp), 4000, controller.signal, () => { });
                    }
    ...
    const overrideCount = Object.keys(overrides).length;
    return (_jsxs("div", { className: "space-y-6 animate-fade-in max-w-5xl", children: [_jsxs("div", { children: [_jsx("h1", { className: "page-header bg-gradient-to-r from-white to-slate-400 bg-clip-text text-transparent", children: "SOA Analysis" }), _jsx("p", { className: "page-sub", children: "AI-powered compliance review \u2014 C1-C29 \u00B7 P1-P10 \u00B7 Consistency \u00B7 Structure." })] }), _jsxs("div", { className: "grid grid-cols-1 md:grid-cols-3 gap-4", children: [_jsx(UploadZone, { num: 1, label: "New SOA *", desc: "The SOA being reviewed", formats: "DOCX or PDF", file: soaDoc, onDrop: handleSoaDrop, onRemove: () => setSoaDoc(null) }), _jsx(UploadZone, { num: 2, label: "Reference SOA", desc: "Optional \u2014 compare structure against this", formats: "DOCX or PDF", file: refDoc, onDrop: handleRefDrop, onRemove: () => setRefDoc(null) }), _jsx(UploadZone, { num: 3, label: "Supporting Documents", desc: "Fact find, fee comparison, CGT workings, etc. Up to 10 files.", formats: "PDF, DOCX, XLSX", multiple: true, files: suppDocs, file: null, onDrop: handleSuppDrop, onRemove: (id) => setSuppDocs((prev) => prev.filter((d) => d.id !== id)) })] }), !result && (_jsxs("div", { className: "card space-y-4 border border-slate-800/80 bg-gradient-to-b from-[#11111d] to-[#0B0B14] shadow-2xl shadow-black/30", children: [error && (_jsxs("div", { className: "flex gap-2 items-start p-3 rounded-lg bg-[#FF6B6B]/10 border border-[#FF6B6B]/20 text-sm text-[#FF6B6B]", children: [_jsx(AlertCircle, { size: 14, className: "shrink-0 mt-0.5" }), _jsx("span", { children: error })] })), !loading ? (_jsxs("div", { className: "grid grid-cols-1 sm:grid-cols-2 gap-3", children: [_jsxs("button", { onClick: () => runReview('quick'), disabled: !soaDoc, className: "relative flex flex-col gap-2 p-5 rounded-2xl border-2 border-[#6B2FD9]/40 bg-[#6B2FD9]/5 hover:bg-[#6B2FD9]/10 transition-all hover:scale-[1.02] disabled:opacity-30", children: [_jsxs("div", { className: "flex items-center gap-2", children: [_jsx(Zap, { size: 16, className: "text-[#A78BFA]" }), _jsx("span", { className: "text-sm font-semibold text-white", children: "Quick Check" })] }), _jsxs("ul", { className: "space-y-1 text-xs text-slate-400 text-left", children: [_jsx("li", { children: "\u2022 FAILs + WARNINGs only \u2014 ~40s" }), _jsx("li", { children: "\u2022 Key consistency + 13 compliance items" }), _jsx("li", { children: "\u2022 P7 + P8 personalisation checks" })] })] }), _jsxs("button", { onClick: () => runReview('full'), disabled: !soaDoc, className: "relative flex flex-col gap-2 p-5 rounded-2xl border-2 border-[#6B2FD9] bg-[#6B2FD9]/10 hover:bg-[#6B2FD9]/15 transition-all hover:scale-[1.02] disabled:opacity-30", children: [_jsxs("div", { className: "flex items-center gap-2", children: [_jsx(Play, { size: 16, className: "text-[#A78BFA]" }), _jsx("span", { className: "text-sm font-semibold text-white", children: "Full Review" })] }), _jsxs("ul", { className: "space-y-1 text-xs text-slate-400 text-left", children: [_jsx("li", { children: "\u2022 All 39+ checks \u2014 C1-C29 \u00B7 P1-P10 \u2014 ~90s" }), _jsx("li", { children: "\u2022 Full consistency + structure analysis" }), _jsx("li", { children: "\u2022 Two-stage review for large document bundles" })] })] })] })) : (_jsxs("div", { className: "space-y-4", children: [_jsxs("div", { className: "flex items-center gap-2 text-sm text-slate-400", children: [_jsx("div", { className: "w-4 h-4 border-2 border-[#6B2FD9] border-t-transparent rounded-full animate-spin" }), _jsx("span", { children: activeSteps[(step - 1) || 0] }), _jsxs("span", { className: "ml-auto text-xs text-slate-600 font-mono", children: [elapsed, "s"] })] }), _jsx("div", { className: "w-full h-2 rounded-full bg-slate-800 overflow-hidden", children: _jsx("div", { className: "h-full bg-gradient-to-r from-[#6B2FD9] to-[#A78BFA] transition-all duration-500", style: { width: `${Math.min(step * 18, 100)}%` } }) }), _jsx("div", { className: "space-y-2", children: activeSteps.map((s, i) => {
                                    const done = i + 1 < step;
                                    const active = i + 1 === step;
                                    return (_jsxs("div", { className: `flex items-center gap-2 text-xs transition-all ${done ? 'text-[#2DD4A0]' : active ? 'text-[#A78BFA]' : 'text-slate-600'}`, children: [done ? _jsx(CheckCircle, { size: 13 }) : active ? _jsx(Clock, { size: 13, className: "animate-pulse" }) : _jsx(Minus, { size: 13 }), s] }, s));
                                }) }), streamText && (_jsx("pre", { className: "bg-[#0B0B14] border border-slate-800 rounded-xl p-4 text-xs text-slate-400 font-mono overflow-auto max-h-48 whitespace-pre-wrap", children: streamText }))] }))] })), result && (_jsxs("div", { className: "card space-y-5 animate-slide-up", children: [_jsxs("div", { className: "flex items-start justify-between gap-3 flex-wrap", children: [_jsxs("div", { children: [_jsx("h2", { className: "text-lg font-bold text-white", style: { fontFamily: "'DM Serif Display', serif" }, children: result.client_name || 'Client' }), _jsxs("p", { className: "text-xs text-slate-500 mt-0.5", children: [result.advice_type, result.adviser_name ? ` · ${result.adviser_name}` : '', result.practice_name ? ` · ${result.practice_name}` : ''] })] }), _jsxs("div", { className: "flex items-center gap-2 flex-wrap", children: [overrideCount > 0 && (_jsxs("span", { className: "text-xs text-[#FFB347] border border-[#FFB347]/30 bg-[#FFB347]/10 px-2 py-1 rounded-full font-medium", children: [overrideCount, " override", overrideCount > 1 ? 's' : '', " applied"] })), _jsxs("button", { onClick: () => exportToWord(result, overrides), className: "inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-700 text-xs font-medium text-slate-300 hover:text-white hover:border-slate-500 transition-colors", children: [_jsx(Download, { size: 12 }), " Export Word"] }), _jsxs("button", { onClick: reset, className: "btn-secondary text-sm", children: [_jsx(RotateCcw, { size: 13 }), " New Review"] })] })] }), _jsx(ScoreTiles, { checks: result.checks, overrides: overrides }), _jsx("div", { className: "border-l-2 border-[#6B2FD9]/60 pl-4 text-sm text-slate-400 whitespace-pre-line", children: result.summary }), _jsx("div", { className: "space-y-0", children: result.checks.map((check) => (_jsx(FindingRow, { check: check, override: overrides[check.id], onSaveOverride: saveOverride, onClearOverride: clearOverride }, check.id))) })] }))] }));
}
