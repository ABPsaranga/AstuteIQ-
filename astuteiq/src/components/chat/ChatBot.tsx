  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      role: 'assistant',
      content: 'Hello! I am the AstuteIQ Assistant. How can I help you today?',
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          id: crypto.randomUUID(),
          role: 'assistant',
          content: 'Unable to contact AstuteIQ Assistant.',
          timestamp: new Date().toISOString(),
        },
      ])
    } finally {
      ...
      {open && (
        <div className="fixed bottom-24 right-6 z-50 flex h-[650px] w-[400px] flex-col overflow-hidden rounded-2xl border border-slate-800 bg-slate-950 shadow-2xl">
          ...
          <div className="flex items-center gap-3 border-b border-slate-800 bg-slate-900 p-4">
            ...
            <div>
              <h3 className="font-semibold text-white">AstuteIQ Assistant</h3>
              <p className="text-xs text-slate-400">Compliance Support</p>
            </div>
          </div>
